...
class CommandBuilder:
    ...
    def build_rsync_command(
        ...
        if bandwidth_limit:
            ...
            if not re.match(r'^\d+[KMG]?$', bandwidth_limit):
                raise ValidationError(f"Invalid bandwidth limit format: {bandwidth_limit}")
    ...
    def build_virsh_command(action: str, vm_name: str, *args: Any) -> str:
        ...
        # Validate action
        valid_actions = {
            "snapshot-create-as", "snapshot-delete", "snapshot-list",
            "dominfo", "list", "start", "shutdown", "destroy"
        }
