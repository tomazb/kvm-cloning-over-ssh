...
class TransferError(KVMCloneError):
    ...
    def __init__(self, message: str, source: str, destination: str) -> None:
        super().__init__(f"Transfer error from {source} to {destination}: {message}", error_code=1006)
        self.source = source
...
class ValidationError(KVMCloneError):
    ...
    def __init__(self, message: str, validation_type: str = "general") -> None:
        super().__init__(f"Validation error ({validation_type}): {message}", error_code=1007)
...
class LibvirtError(KVMCloneError):
    ...
    def __init__(self, message: str, operation: str = "unknown") -> None:
        super().__init__(f"Libvirt error during {operation}: {message}", error_code=1008)
...
class SSHError(KVMCloneError):
    ...
    def __init__(self, message: str, host: str, operation: str = "connection") -> None:
        super().__init__(f"SSH error on {host} during {operation}: {message}", error_code=1009)
        self.host = host
        self.operation = operation
...
class AuthenticationError(KVMCloneError):
    ...
    def __init__(self, message: str, host: str, auth_method: str = "key") -> None:
        super().__init__(f"Authentication failed for {host} using {auth_method}: {message}", error_code=1010)
        self.host = host
...
class PermissionError(KVMCloneError):
    ...
    def __init__(self, message: str, resource: str, operation: str = "access") -> None:
        super().__init__(f"Permission denied for {operation} on {resource}: {message}", error_code=1011)
...
class TimeoutError(KVMCloneError):
    ...
    def __init__(self, message: str, operation: str, timeout: int) -> None:
        super().__init__(f"Timeout during {operation} after {timeout}s: {message}", error_code=1012)
        self.operation = operation
...
class OperationCancelledError(KVMCloneError):
    ...
    def __init__(self, operation_id: str, operation_type: str) -> None:
        super().__init__(f"{operation_type} operation {operation_id} was cancelled", error_code=1013)
...
class IntegrityError(KVMCloneError):
    ...
    def __init__(self, message: str, file_path: str) -> None:
        super().__init__(f"Integrity check failed for {file_path}: {message}", error_code=1014)
...
class DiskSpaceError(InsufficientResourcesError):
    ...
    def __init__(self, required: int, available: int, path: str) -> None:
        message = f"Required {required} bytes, only {available} bytes available at {path}"
...
class NetworkError(KVMCloneError):
    ...
    def __init__(self, message: str, network_name: str, operation: str = "configuration") -> None:
        super().__init__(f"Network error in {network_name} during {operation}: {message}", error_code=1015)
