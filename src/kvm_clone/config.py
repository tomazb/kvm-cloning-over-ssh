...
class AppConfig(BaseModel):
    ...
    default_timeout: int = Field(default=30, gt=0, description="Default SSH timeout in seconds")
    log_level: str = Field(default="INFO", description="Logging level")
    known_hosts_file: Optional[str] = None
    
    # Default values for operations
    default_parallel_transfers: int = Field(default=4, gt=0, description="Number of parallel transfers")
...
class ConfigLoader:
    ...
    def _load_from_file(self, path: str) -> AppConfig:
        ...
        try:
            ...
            self.logger.error(f"Failed to parse configuration file {path}: {e}", path=path, exc_info=True)
            raise ConfigurationError(f"Failed to parse configuration file: {e}")
        except ValueError as e:
            # Pydantic validation error
            self.logger.error(f"Invalid configuration in {path}: {e}", path=path, exc_info=True)
            raise ConfigurationError(f"Invalid configuration: {e}")
        except Exception as e:
            self.logger.error(f"Failed to load configuration from {path}: {e}", path=path, exc_info=True)
