...
class TestSecurityFixes:
    ...
    def test_safe_command_building(self):
        ...
        cmd = CommandBuilder.build_safe_command(
            "cp {source} {dest}", source="/path/to/file", dest="/dest/path"
        )
    ...
    def test_virsh_command_building(self):
        ...
        cmd = CommandBuilder.build_virsh_command(
            "snapshot-create-as", "test-vm", "snap; rm -rf /", "desc`whoami`"
        )
        ...
        with pytest.raises(ValidationError):
            CommandBuilder.build_virsh_command("dangerous-action", "test-vm")
    ...
    def test_rsync_command_building(self):
        ...
        cmd = CommandBuilder.build_rsync_command(
            source_path="/source/path", dest_path="/dest/path"
        )
        ...
        cmd = CommandBuilder.build_rsync_command(
            source_path="/source; rm -rf /", dest_path="/dest`whoami`"
        )
        ...
        # Remote rsync
        cmd = CommandBuilder.build_rsync_command(
            source_path="/source/path", dest_path="/dest/path", dest_host="remote.host"
        )
        assert "remote.host:/dest/path" in cmd

        # With bandwidth limit
        cmd = CommandBuilder.build_rsync_command(
            source_path="/source/path", dest_path="/dest/path", bandwidth_limit="100M"
        )
